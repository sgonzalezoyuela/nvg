...
nixInfo.lze.load {
  ...
  -- [[ Java ]]
  {
    ...
    after = function(_)
      ...
      -- Also re-attach on future java buffers
      vim.api.nvim_create_autocmd("FileType", {
        pattern = "java",
        callback = function()
          require('jdtls').start_or_attach(jdtls_config)
  ...
  },
  {
    ...
    after = function (plugin)
      require('lint').linters_by_ft = {
        go = nixInfo(nil, "settings", "cats", "go") and { "golangcilint" } or nil,
        kotlin = nixInfo(nil, "settings", "cats", "kotlin") and { "ktlint" } or nil,
      }
